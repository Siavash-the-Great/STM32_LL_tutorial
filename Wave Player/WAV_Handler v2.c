...
/********///Functions:
	...
	bool wave_readHeader(FIL *filptr, struct Wave_file *wav1ptr, volatile uint32_t **DACRegADS, uint32_t *DMA_DataLength)
	{
		FRESULT fr;     /* FatFs return code */
		UINT br;		//file read
		...
		else
		{
			...
			memcpy(wav1ptr->ChunkID, &headerBuffer[0], 4);
			memcpy(temp, &headerBuffer[4], 4);
			wav1ptr->ChunkSize = array2Int(temp);
			memcpy(wav1ptr->Format, &headerBuffer[8], 4);
				//The "fmt" sub-chunk	
			memcpy(wav1ptr->Subchunk1ID, &headerBuffer[12], 4);
			memcpy(temp, &headerBuffer[16], 4);
			wav1ptr->Subchunk1Size = array2Int(temp);
			memcpy(temp, &headerBuffer[20], 2);
			wav1ptr->AudioFormat = array2Short(temp);	
			memcpy(temp, &headerBuffer[22], 2);
			wav1ptr->NumChannels = array2Short(temp);	
			memcpy(temp, &headerBuffer[24], 4);
			wav1ptr->SampleRate = array2Int(temp);
			memcpy(temp, &headerBuffer[28], 4);
			wav1ptr->ByteRate = array2Int(temp);	
			memcpy(temp, &headerBuffer[32], 2);
			wav1ptr->BlockAlign	= array2Short(temp);	
			memcpy(temp, &headerBuffer[34], 2);
			wav1ptr->BitsPerSample = array2Short(temp);	
				//The "data" sub-chunk
			memcpy(wav1ptr->Subchunk2ID, &headerBuffer[16 + wav1ptr->Subchunk1Size + 4], 4);
			memcpy(temp, &headerBuffer[16 + wav1ptr->Subchunk1Size + 8], 4);
			wav1ptr->Subchunk2Size	= array2Int(temp);	

		
			if( (strncmp(wav1ptr->ChunkID, "RIFF", 4) != 0) ||  (strncmp(wav1ptr->Format , "WAVE", 4) != 0) ){
				return false;
			}
			else
			{
				// set read/write pointer after the header information to read data 
				uint16_t headerEnd = 16 + wav1ptr->Subchunk1Size + 12;		// pointer to the end of header of wav file	
				...
				* memory/peripheral data alignment, based on bits per sample & Number of channels */
				...
				if((wav1ptr->BitsPerSample == 8) && (wav1ptr->NumChannels == 1))
				{
				...
				else if((wav1ptr->BitsPerSample == 16) && (wav1ptr->NumChannels == 1))
				{
				...
				else if((wav1ptr->BitsPerSample == 8) && (wav1ptr->NumChannels == 2))
				{
				...
				else if((wav1ptr->BitsPerSample == 16) && (wav1ptr->NumChannels == 2))
				{
...
//		if(BufferAction == 0)
	...
	void wave_play(char *fileName)
	{
		...
		struct Wave_file wav1;					//struct a new header		
		...
		isWAV = wave_readHeader(&fil, &wav1, &DACRegADS, &DMA_DataLength);
		
		if(isWAV){
		printf("playing the file... \r\n");	
		wave_setSampleRate(wav1.SampleRate);
		wave_read(&fil, wavBuffer, DACRegADS, DMA_DataLength);
		while(1)
		{
			wave_IsEndofFile(&fil, wavBuffer, wav1.BitsPerSample);	
		}
		}
		else{
